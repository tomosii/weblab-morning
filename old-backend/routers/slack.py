...
async def slack_morning_command(request: Request):
    ...
    try:
        ...
        print(f"Error in /slack/morning-command: {e}")
    ...
    try:
        ...
        if modal_title == "朝活の参加表明":
            ...
            commit_dates_block = next(
                block
                for block in blocks
                if block.get("block_id") == "commit-dates-block"
            )
            ...
            absent_reason = answers["absent-reason-block"]["absent-reason-action"][
                "value"
            ]
            today = datetime.datetime.now(ZoneInfo("Asia/Tokyo")).date()
            absent_date = today + datetime.timedelta(days=1)
        ...
        print(f"Error in /slack/interactivity: {e}")
        raise HTTPException(status_code=500, detail="Internal server error.")
